...
from ..utils import jsondump, writefile
class EC:
	...
	def train(self, data):
		...
		dataset, etri, conll = self.datamodule.generate_training_data(data)
		train_dataset = dataset[:int(len(dataset) * 0.9)]
		dev_dataset = dataset[int(len(dataset) * 0.9):]

		writefile(conll[int(len(conll) * 0.9):], self.config["conll_eval_path"], processor=lambda x: x+"\n")
		...
		self.model.start_enqueue_thread(self.session, train_dataset)
		...
		while True:
			tf_predictions, tf_loss, tf_global_step, _ = self.session.run(
			...
			if tf_global_step % eval_frequency == 0:
				saver.save(self.session, os.path.join(log_dir, "model"), global_step=tf_global_step)
				eval_summary, eval_f1 = self.model.evaluate(self.session, dev_dataset)
	...
	def predict_coreference(self, el_input):
		jsonline_input, etri, _ = self.datamodule.convert_data(el_input)
		prediction = []
