...
import random
import numpy as np
from .reader import CoNLL03Reader, CoNLLFormReader
...
def load_alphabets(alphabet_directory):
    # load alphabet from default alphabet directory(data/er/alphabets)
    if not os.path.isdir(alphabet_directory): raise Exception("Alphabet not initialized")
...
def read_data(source_path, word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, ner_alphabet, max_size=None,
    ...
    counter = 0
    reader_class = CoNLL03Reader if type(source_path) is str else CoNLLFormReader
    reader = reader_class(source_path, word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, ner_alphabet)
