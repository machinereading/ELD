...
class MulRelRanker(LocalCtxAttRanker):
    ...
    def forward(self, token_ids, tok_mask, entity_ids, entity_mask, p_e_m, gold=None):
        ...
        if self.use_local:
            ...
            local_ent_scores = Variable(torch.zeros(n_ments, n_cands).to(self.device), requires_grad=False)
        ...
        if self.use_pad_ent:
            ...
            local_ent_scores = torch.cat([local_ent_scores,
                                          Variable(torch.zeros(1, n_cands).to(self.device), requires_grad=False)],
                                         dim=0)
            n_ments += 1

            if self.oracle:
                tmp = Variable(torch.zeros(1, 1).to(self.device).long())
                gold = torch.cat([gold, tmp], dim=0)

        if self.use_local_only:
            inputs = torch.cat([Variable(torch.zeros(n_ments * n_cands, 1).to(self.device)),
        ...
        if n_ments == 1:
            ...
            ment_pos = torch.arange(0, n_ments).long().to(self.device)
            ...
            eye = Variable(torch.eye(n_ments).to(self.device)).view(1, n_ments, n_ments)
            ...
            if gold is None:
                ...
                onehot_gold = Variable(torch.zeros(n_ments, n_cands).to(self.device)).scatter_(1, gold, 1)
