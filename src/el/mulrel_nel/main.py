...
def main():
    ...
    tta_datasets = [('test', conll.tta_test),]
    ...
    if args.mode == 'train':
        ...
        for di, (dname, data) in enumerate(dev_datasets):
            ...
            print(dname, utils.tokgreen('micro F1: ' + str(D.eval_to_log(org_dev_datasets[di][1], predictions))))
    elif args.mode == "test":
        print("test")
        org_tta_datasets = tta_datasets  # + [('aida-train', conll.train)]
        tta_datasets = []
        for dname, data in org_tta_datasets:
            tta_datasets.append((dname, ranker.get_data_items(data, predict=True)))
            print(dname, '#dev docs', len(tta_datasets[-1][1]))

        vecs = ranker.model.rel_embs.cpu().data.numpy()

        for di, (dname, data) in enumerate(tta_datasets):
            # if di == 0:
            #     continue 
            ranker.model._coh_ctx_vecs = []
            predictions = ranker.predict(data)
            print(dname, utils.tokgreen('micro F1: ' + str(D.eval_to_log(org_tta_datasets[di][1], predictions))))



# HARD-CODED!!!!!
args.mode = "test"
args.model_path = "unk_0.1"
...
def test(conll_str, tsv_str):
    global ranker
    tta_datasets = [("test", D.generate_dataset_from_str(conll_str, tsv_str))]
    # print("test")
    ...
    for di, (dname, data) in enumerate(tta_datasets):
        ...
        ranker.model._coh_ctx_vecs = []
        predictions = ranker.predict(data)
        return D.eval_to_log(org_tta_datasets[di][1], predictions)
