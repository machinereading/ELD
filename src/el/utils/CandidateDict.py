...
class CandidateDict():
	...
	def __getitem__(self, query):
		...
		if query in self.surface_dict:
			...
			elem += [(ent, score * self.link_modifier) for ent, score in self.surface_dict[query] if
			         ent != e.exact_entity]
		if len(elem) > 0:
	...
	# result = [(ent, self.entity_set[ent], score) for ent, score in filter(lambda x: x[0] != "", elem)]
	#
	# if flag:
	...
	def load_entity_from_file(cls, f, d=None):
		...
		for line in f.readlines():
			result.entity_set[line.strip().split("\t")[0].replace("ko.dbpedia.org/resource/", "")] = len(
				result.entity_set)
