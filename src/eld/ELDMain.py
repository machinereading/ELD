...
class ELDMain:
	...
	def train(self):
		train_batch = DataLoader(dataset=None, batch_size=32, shuffle=True, num_workers=4)
		dev_batch = DataLoader(dataset=None, batch_size=32, shuffle=False, num_workers=4)
		tqdmloop = tqdm(range(1, self.args.epochs + 1))
		optimizer = torch.optim.Adam(self.transformer.parameters(), lr=1e-4, weight_decay=1e-4)
		gold_corpus = None  # TODO preload gold corpus
		max_score = 0
		max_score_epoch = 0
